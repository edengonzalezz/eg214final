...
moving_average <- function(stream_df) {
  ...
  # Iterate over the rows of the result dataframe
  for (i in 1:nrow(result)) {
    # Create variables for the start and end of the current window
    w1 <- result$window_start[i]
    w2 <- result$window_start[i] + weeks(9)

    # Create a logical vector, called "in_window", that keeps samples that are inside the window
    ...
    # Drop NAs to not impact mean calculations
